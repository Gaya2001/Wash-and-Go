...
const PendingRequests = () => {
  ...
  const pendingRequests = requests.filter(
    (request) => request.States === "complete"
  );
  console.log(pendingRequests);

  const handleDetailsClick = (userId) => {
    navigate(`/Completed-details/${userId}`);
  };
        ...
        <div className="flex-1 p-10">
          <div className="bg-green-100 p-8 rounded-lg h-screen">
            ...
            {pendingRequests.length === 0 ? (
              ...
              <table className="min-w-full bg-green-200 text-gray-700 rounded-lg  table-auto">
                <thead className="sticky top-0  bg-green-200 text-gray-700 rounded-lg">
                  <tr>
